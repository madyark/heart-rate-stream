...
The `dbt init` command was used to initialize the dbt project. Use the `dbt deps` command to ensure all required packages defined in `packages.yml` are installed and ready for use. The `dbt build` command compiles and constructs all of the seeds, tests, and models defined in the project, shaping transformed data into the desired format for subsequent analysis and reporting. The commands `dbt docs generate` and `dbt docs serve` generate and serve a display of data lineage and schema documentation of the dbt models.
